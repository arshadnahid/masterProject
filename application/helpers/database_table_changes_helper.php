...
if (!function_exists('sales_details_table')) {

    function sales_details_table($option = '')
    {
        ...
        foreach ($sales_invoice_array as $key => $csm) {
            $query = $this->db->field_exists($csm['Field'], 'sales_details');
            if ($query != TRUE) {
                $this->load->dbforge();
                $fields = array(
                    $csm['Field'] => array(
        ...
        $CI =& get_instance();
        $CI->load->database();
        $sql = "SELECT COUNT(Accounts_VoucherMst_AutoID) AS inserted_voucher_number FROM ac_accounts_vouchermst WHERE AccouVoucherType_AutoID in(3,5)";
        ...
        $purchase_invoice_no = "JV" . date('y') . date('m') . str_pad(($totalPurchases) + 1, 4, "0", STR_PAD_LEFT);
