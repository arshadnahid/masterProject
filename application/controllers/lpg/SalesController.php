...
//print_r($data['editInvoice']);exit;
    ...
    public function customer_ajax()
    {
        ...
        foreach ($query['invoice_list'] as $key => $row):
            ...
            $result .= '<td class="text-center"><input type="hidden" name="invoiceAmount[]" id="invoice_amount_' . $row['sales_invoice_id'] . '" class="" value="' . $row['invoice_amount'] . '"/><input type="hidden" name="invoiceID[]" id="invoice_id_' . $row['sales_invoice_id'] . '" class="invoice_id" value="' . $row['sales_invoice_id'] . '"/><input type="hidden" name="invoiceNo[]" id="invoice_no_' . $row['invoice_no'] . '" class="invoice_id" value="' . $row['sales_invoice_id'] . '"/><input id="paymentAmount_' . $row['sales_invoice_id'] . '" type="text" onkeyup="checkOverAmount(' . $row['sales_invoice_id'] . ')"  type="text" class="form-control amount" name="amount[]"  placeholder="0.00" autocomplete="off" onclick="this.select();"></td>';
            $result .= '</tr>';
            //endif;
            //endif;
        endforeach;
