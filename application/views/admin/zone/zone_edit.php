                            ...
                            <div class="col-sm-6">
                                <select  name="divisionId" onchange="getDivisionId(this.value)" class="chosen-select form-control" id="form-field-select-3" data-placeholder="Search Division">
                                    ...
                                    <?php foreach ($divisionList as $key => $value): ?>
                                        <option <?php
                                    if (!empty($zoneList->divisionId) && $zoneList->divisionId == $value->id) {
                                        echo "selected";
                                    }
...
<?php endforeach; ?>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <div class="col-sm-6">
                                    <input type="text" id="form-field-1" name="zone_title"  value="<?php echo $zoneList->zone_title; ?>" class="form-control" placeholder="Zone Name" />
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    $.ajax({
        type: 'POST',
        url: url,
        data:{ 'divisionId': '<?php echo $zoneList->divisionId; ?>','districtId': '<?php echo $zoneList->districtId; ?>'},
    ...
    $.ajax({
        type: 'POST',
        url: url,
        data:{ 'districtid': '<?php echo $zoneList->districtId; ?>','thanaId': '<?php echo $zoneList->thanaId; ?>'},
        success: function (data)
        {
            $('#thanaList').chosen();
