            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                foreach ($_POST['slNo'] as $key => $value) {
                    ...
                    if ($category_id == 2 && $_POST['is_package_' . $value] == 0) {
                        ...
                        $productCost = $this->Sales_Model->productCostNew($packageEmptyProductId, $this->dist_id);
                    ...
                    if (isset($_POST['returnproduct_' . $value])) {
                        ...
                        if ($category_id == 2 && ($returnable_quantity == $return_quentity)) {
                            if ($profitOrLose < 0) {
                                $GR_Amount = ($profitOrLose * -1);
                            } else {
                                $GR_Amount = ($profitOrLose);
                            }
                            if ($GR_Amount > 0) {
                                $accountingDetailsTableCustomerReceivable['Accounts_VoucherMst_AutoID'] = $accountingVoucherId;
                                $accountingDetailsTableCustomerReceivable['TypeID'] = $this->TypeDR;
                                $accountingDetailsTableCustomerReceivable['CHILD_ID'] = $this->config->item("EmptyCylinderProfitLoss");//$CustomerReceivable->id;//'25';
                                $accountingDetailsTableCustomerReceivable['GR_DEBIT'] = $GR_Amount;
                                $accountingDetailsTableCustomerReceivable['GR_CREDIT'] = 0;
                                ...
                                $accountingDetailsTableCustomerReceivable['GR_DEBIT'] = 0;
                                $accountingDetailsTableCustomerReceivable['GR_CREDIT'] = $GR_Amount;
