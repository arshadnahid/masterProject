...
class ImportExcelController extends CI_Controller
{
    ...
    public function setupImportExcelCustomer()
    {
        ...
        foreach ($object->getWorksheetIterator() as $worksheet) {
            ...
            for ($row = 2; $row <= $highestRow; $row++) {
                ...
                $data['customerName'] = $customerName=trim($worksheet->getCellByColumnAndRow(0, $row)->getValue());
                $data['customerPhone'] = $worksheet->getCellByColumnAndRow(1, $row)->getValue();
                $data['customerEmail'] = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
                $data['customerAddress'] = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
                $data['customerType'] = 1;
                $data['dist_id'] = $this->dist_id;
                $data['updated_by'] = $this->admin_id;
                $data['root_id'] = 1;
                ...
                $condition = array(
                    ...
                    'customerPhone' => $worksheet->getCellByColumnAndRow(1, $row)->getValue(),
                );
    ...
    public function setupImportExcel()
    {
        if (isPostBack()) {
                ...
                while (($readRowData = fgetcsv($importFile, 1000, ",")) !== false) {
                    if ($row != 0):
                        ...
                        if (!empty($data['customerName'])):
                            ...
                            $dataCoa['parent_id'] = $ac_account_ledger_coa_info->id;
                            $dataCoa['code'] = $newCode;
                            $dataCoa['parent_name'] = $readRowData[0] . ' [ ' . $dynamicId . ' ]  Empty Cylinder Due';
                            $dataCoa['status'] = 1;
                            $dataCoa['posted'] = 1;
                            $dataCoa['level_no'] = $level_no + 1;
                            $dataCoa['related_id'] = $insertID;
